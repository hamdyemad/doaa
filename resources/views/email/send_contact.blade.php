    ...
    <tbody>
      ...
      <tr>
        <td style="width:50%;padding:20px;vertical-align:top">
            @if($data['topic'])
                <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px"> الاسم :  </span>{{ $data['name'] }}</p>
            @endif
            @if($data['txt'])
                <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px"> البريد االالكتروني :  </span>{{ $data['email'] }}</p>
            @endif
        </td>
      </tr>
      <tr>
